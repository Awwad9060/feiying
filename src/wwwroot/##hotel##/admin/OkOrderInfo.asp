...
<tr>
	<td width="639" height="30" valign="middle" style="font-size:14px; font-weight:bold; color:#3300FF">　處理好訂單</td>
</tr>
  <tr> 
    <td>
<%
  ...
  if search_word="" then
  sql="select * from orderb where flag=2 order by id desc"
   else
  sql="select * from orderb where c_id like '%"&search_word&"%' and  flag=2 order by id desc"
  end if
...
<% 
 ...
 %>
          <tr align="center"> 
            <td colspan="9" height="26" bgcolor="#99CCFF"><font color="#FF0000">沒有數據</font></td>
          </tr>
          <%else
