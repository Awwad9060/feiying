...
public partial class console_airport_airport_list : System.Web.UI.Page
{
    ...
    protected void GetShopNewslist()
    {
        ...
        string str = @"select top 100 percent * from FY_Airport where 1=1 ";

        if (txtsearch.Value.Trim() != "")
        {
            str += " and chName like '%" + txtsearch.Value.Trim() + "%' or airportCode like '%" + txtsearch.Value.Trim() + "%'";
        }

        str += " order by apId desc";
    ...
    protected void btndel_Click(object sender, EventArgs e)
    {
        ...
        str.Append("delete from FY_Airport where apId in (" + Request.Form["chk"] + ");");
        ...
        using (SqlTransaction trans = conn.BeginTransaction())
        {
            try
            {
                ...
                trans.Commit();
                scripthelp.AlertAndRedirect("操作成功！", "airport_list.aspx", this.Page);
            }
            catch
            {
                trans.Rollback();
                scripthelp.AlertAndRedirect("操作失败！", "airport_list.aspx", this.Page);
            }
            finally
            {
                conn.Close();
