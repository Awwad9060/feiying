...
public partial class console_prizes_lottery_list : System.Web.UI.Page
{
    ...
    protected void Getlotlist()
    {
        ...
        string str = @"select top 100 percent FY_Prizes.pId as prId, FY_Prizes.pName as pName,FY_lottery.* from FY_lottery left join FY_Prizes on FY_lottery.pId=FY_Prizes.pId  order by lotId desc";
    ...
    protected void btndel_Click(object sender, EventArgs e)
    {
        ...
        str.Append("delete from FY_lottery where lotId in (" + Request.Form["chk"] + ");");
        ...
        using (SqlTransaction trans = conn.BeginTransaction())
        {
            try
            {
                ...
                trans.Commit();
                scripthelp.AlertAndRedirect("操作成功！", "lottery_list.aspx", this.Page);
            }
            catch
            {
                trans.Rollback();
                scripthelp.AlertAndRedirect("操作失败！", "lottery_list.aspx:", this.Page);
            }
            finally
            {
                conn.Close();
