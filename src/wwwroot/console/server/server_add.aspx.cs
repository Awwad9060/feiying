...
public partial class console_server_server_add : System.Web.UI.Page 
{
    static int id = 0;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            if (Request.QueryString["id"] != null)
            {
                ...
                this.txt_name.Value = ds.Tables[0].Rows[0]["name"].ToString();
                this.txt_title.Value = ds.Tables[0].Rows[0]["title"].ToString();
                this.txt_link.Value = ds.Tables[0].Rows[0]["link"].ToString();

                this.isUploadPic.Text = "已上传：<img src=../../" + ds.Tables[0].Rows[0]["pic"].ToString() + " height='80' width='80'>";
                this.oldPic.Value = ds.Tables[0].Rows[0]["pic"].ToString();

                this.lblTitle.Text = "修改特色服务信息";
            }
            else
            {
                id = 0;
                this.lblTitle.Text = "添加特色服务信息";
            }
        }
    }
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        ...
        if (id == 0)
        {
            ...
            if (FileUpload_picPath.HasFile)
            {
                if (FileUploadOK(out picPath))
                {
                    ...
                    str = "insert into FY_server(name,title,pic,link) values('" + this.txt_name.Value.Trim() + "','" + this.txt_title.Value.Trim() + "','" + picPath.Trim() + "','"+this.txt_link.Value+"')";

                    int inserttag = SqlHelper.ExecuteNonQuery(SqlHelper.ConnString_insert, CommandType.Text, str);

                    if (inserttag > 0)
                    {
                        Response.Write("<script>alert('操作成功！');location.href='server_list.aspx'</script>");
                    }
                    else
            ...
            {
                str = "insert into FY_server(name,title,link) values('" + this.txt_name.Value.Trim() + "','" + this.txt_title.Value.Trim() + "','" + this.txt_link.Value + "')";
        ...
        else
        {
            ...
            if (FileUpload_picPath.HasFile)
            {
                if (FileUploadOK(out picPath))
                {
                    str = "update FY_server set name='" + this.txt_name.Value.Trim() + "',title='" + this.txt_title.Value.Trim() + "',pic='" + picPath.Trim() + "',link='"+this.txt_link.Value+"' where id=" + id ;                  
            ...
            else
            {
                ...
                str = "update FY_server set name='" + this.txt_name.Value.Trim() + "',title='" + this.txt_title.Value.Trim() + "',link='" + this.txt_link.Value + "' where id=" + id;                  
