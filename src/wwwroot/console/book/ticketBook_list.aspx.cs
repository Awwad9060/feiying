...
public partial class console_book_ticketBook_list : System.Web.UI.Page
{
    ...
    protected void btndel_Click(object sender, EventArgs e)
    {
        ...
        str.Append("delete from FY_TicketBook where bId in (" + Request.Form["chk"] + ");");
        ...
        using (SqlTransaction trans = conn.BeginTransaction())
        {
            try
            {
                ...
                trans.Commit();
                scripthelp.AlertAndRedirect("操作成功！", "ticketBook_list.aspx", this.Page);
            }
            catch
            {
                trans.Rollback();
                scripthelp.AlertAndRedirect("操作失败！", "ticketBook_list.aspx", this.Page);
            }
            finally
            {
                conn.Close();
