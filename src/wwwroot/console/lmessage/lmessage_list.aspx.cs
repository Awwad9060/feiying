...
public partial class console_lmessage_lmessage_list : System.Web.UI.Page
{
    ...
    protected void GetLeavelist()
    {
        ...
        string str = @"select top 100 percent * from FY_LeaveMessage ";
        str += " where 1=1  ";

        str += " order by FY_LeaveMessage.lId desc";
    ...
    protected void btndel_Click(object sender, EventArgs e)
    {
        ...
        str.Append("delete from FY_LeaveMessage where lId in (" + Request.Form["chk"] + ");");
        ...
        using (SqlTransaction trans = conn.BeginTransaction())
        {
            try
            {
                ...
                trans.Commit();
                scripthelp.AlertAndRedirect("操作成功！", "lmessage_list.aspx", this.Page);
            }
            catch
            {
                trans.Rollback();
                scripthelp.AlertAndRedirect("操作失败！", "lmessage_list.aspx", this.Page);
            }
            finally
            {
                conn.Close();
