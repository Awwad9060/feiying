...
public partial class cms_ticket_ticket_add2 : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        AjaxPro.Utility.RegisterTypeForAjax(typeof(cms_ticket_ticket_add2));
    }
    ...
    protected void bind_allInfo()
    {
        if (!IsPostBack)
        {
            ...
            txtStartDate.Value = DateTime.Now.ToString("HH:mm");
            txtEndDate.Value = DateTime.Now.AddHours(2).AddMinutes(30).ToString("HH:mm");

            txtStartDate2.Value = DateTime.Now.AddHours(4).AddMinutes(30).ToString("HH:mm");
            txtEndDate2.Value = DateTime.Now.AddHours(6).AddMinutes(30).ToString("HH:mm");

            txtStartDate5.Value = DateTime.Now.AddHours(5).AddMinutes(30).ToString("HH:mm");
            txtEndDate5.Value = DateTime.Now.AddHours(7).ToString("HH:mm");

            txtStartDate3.Value = DateTime.Now.ToString("HH:mm");
            txtEndDate3.Value = DateTime.Now.AddHours(2).AddMinutes(30).ToString("HH:mm");

            txtStartDate4.Value = DateTime.Now.AddHours(4).AddMinutes(30).ToString("HH:mm");
            txtEndDate4.Value = DateTime.Now.AddHours(6).AddMinutes(30).ToString("HH:mm");

            txtStartDate6.Value = DateTime.Now.AddHours(5).AddMinutes(30).ToString("HH:mm");
            txtEndDate6.Value = DateTime.Now.AddHours(7).ToString("HH:mm");
            ////时间初始化
    ...
    {
        int ticketType = 0, tripType = 0, returnTip = 0, isPass = 0, isRecom = 0, isTransfer = 0;
        ...
        if (id == 0)
        {
            tripType = Request.Form["selTripType"] != null ? Convert.ToInt32(Request.Form["selTripType"]) : 0;
            isTransfer = Request.Form["selZz"] != null ? Convert.ToInt32(Request.Form["selZz"]) : 0;

            if (Request.Form["txtUntaxPrice_more"] != null)
            {
                string[] untaxPrice_more = Convert.ToString(Request.Form["txtUntaxPrice_more"]).Split(',');//票价
                string[] faxPrice_more = Convert.ToString(Request.Form["txtFaxPrice_more"]).Split(',');//税费
                ...
                string title = "";//标题
                string bagPrice = "";//免费行李额
                int shortstayDate_more = 0;//最短停留天数
                int longstayDate_more = 0;//最长停留天数
                int attPrice = 0;//旺季附加费
                int childPrice = 0;//儿童折扣价
                string fromsaleDate_more = Request.Form["fromsaleDate_more"] != null ? Request.Form["fromsaleDate_more"] : DateTime.Now.ToString("yyyy-MM-dd");//售票开始时间
                string tosaleDate_more = Request.Form["tosaleDate_more"] != null ? Request.Form["tosaleDate_more"] : DateTime.Now.ToString("yyyy-MM-dd");//售票截止时间
                string returnMoney = "";//退款条款
                string updateProvision = "";//改期条款
                int score = 100;//积分
                int addScore = 100;//积分+
                int zongping = 3;//综评
                int xiai = 2;//喜爱
                ...
                commandText2 += "shortstayDate,longstayDate,attPrice,childPrice,fromsaleDate,tosaleDate,fromtripDate,totripDate,returnMoney,updateProvision,limitProvision,addtime,updatetime,isChear,isRecom,score,addScore,zongping,xiai,xiaofei,manyi,fuwu,IsLimited,IsTransfer,FaxPrice)";
                commandText2 += "values('" + title + "'," + fromCity.Value.Trim() + "," + getStateIdByName(txtSearch_tocity.Value) + "," + getToCityIdByName(txtSearch_tocity.Value) + "," + ticketType + "," + tripType + "," + getAirlineIdByName(txtSearchAline.Value) + ",";
                commandText2 += "" + getAirPortIdByName(txtSearch_fromport.Value) + "," + getAirPortIdByName(txtSearch_toport.Value) + ",'" + Sys.filterSQLStr(txtSearch_relayport.Value) + "','" + bagPrice + "'," + returnTip + ",";
                commandText2 += "'" + Sys.filterSQLStr(engerTicket.Value.Trim()) + "','" + Sys.filterSQLStr(seatShip.Value.Trim()) + "','" + Sys.filterSQLStr(untaxPrice_more[0]) + "',";
                commandText2 += "'" + shortstayDate_more + "','" + longstayDate_more + "','" + attPrice + "',";
                commandText2 += "'" + childPrice + "','" + Sys.filterSQLStr(fromsaleDate_more) + "','" + Sys.filterSQLStr(tosaleDate_more) + "',";
                commandText2 += "'" + Sys.filterSQLStr(fromsaleDate_more) + "','" + Sys.filterSQLStr(tosaleDate_more) + "','" + returnMoney + "',";
                commandText2 += "'" + updateProvision + "','" + Sys.filterSQLStr(FCKeditor.Value) + "',getdate(),getdate()," + isPass + "," + isRecom + "," + score + "," + addScore + "," + zongping + "," + xiai + "," + xiaofei + "," + manyi + "," + fuwu + "," + IsLimited + "," + isTransfer + "," + Sys.filterSQLStr(faxPrice_more[0]) + ")";
                commandText2 += ";SELECT @@IDENTITY AS 自增的id号;";
                int newId = Convert.ToInt32(SqlHelper.ExecuteScalar(SqlHelper.ConnString_select, CommandType.Text, commandText2));
                ...
                else
                {
                    ...
                    try
                    {
                        //string strSql = "SELECT @@IDENTITY AS 自增的id号;";
                        //int newId = Convert.ToInt32(SqlHelper.ExecuteScalar(SqlHelper.ConnString_select, CommandType.Text, strSql));

                        string strInsertDetail = "insert into FY_Ticket_Detail(tid,tType,airLine,flightNumber,planeType,relayPort,formdate,todate,adddate,updatedate) values('" + newId + "','{0}','{1}','{2}','{3}','{4}','{5}','{6}','" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "','" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "');";
                        ...
                        if (newId > 0)
                        {
                            if (tripType == 0)
                            {
                                ...
                                if (isTransfer == 0)//不中转
                                {
                                    sSql += string.Format(strInsertDetail, 0, getAirlineIdByName(txtSearchAline.Value), Sys.filterSQLStr(txtFlightNumber.Value), Sys.filterSQLStr(txtPlaneType.Value),"", Sys.filterSQLStr(txtStartDate.Value), Sys.filterSQLStr(txtEndDate.Value));
                                }
                                else if (isTransfer == 2)//单程二次中转
                                {
                                    sSql += string.Format(strInsertDetail, 0, getAirlineIdByName(txtSearchAline.Value), Sys.filterSQLStr(txtFlightNumber.Value), Sys.filterSQLStr(txtPlaneType.Value), "", Sys.filterSQLStr(txtStartDate.Value), Sys.filterSQLStr(txtEndDate.Value));
                                    sSql += string.Format(strInsertDetail, 1, getAirlineIdByName(txtSearchAline2.Value), Sys.filterSQLStr(txtFlightNumber2.Value), Sys.filterSQLStr(txtPlaneType2.Value), Sys.filterSQLStr(txtSearch_relayport2.Value), Sys.filterSQLStr(txtStartDate2.Value), Sys.filterSQLStr(txtEndDate2.Value));
                                    sSql += string.Format(strInsertDetail, 2, getAirlineIdByName(txtSearchAline5.Value), Sys.filterSQLStr(txtFlightNumber5.Value), Sys.filterSQLStr(txtPlaneType5.Value), Sys.filterSQLStr(txtSearch_relayport3.Value), Sys.filterSQLStr(txtStartDate5.Value), Sys.filterSQLStr(txtEndDate5.Value));
                                }
                                else//单程一次中转
                                {
                                    sSql += string.Format(strInsertDetail, 0, getAirlineIdByName(txtSearchAline.Value), Sys.filterSQLStr(txtFlightNumber.Value), Sys.filterSQLStr(txtPlaneType.Value),"", Sys.filterSQLStr(txtStartDate.Value), Sys.filterSQLStr(txtEndDate.Value));
                            ...
                            if (tripType == 1)
                            {
                                if (isTransfer == 0)//往返不中转
                                {
                                    sSql += string.Format(strInsertDetail, 0, getAirlineIdByName(txtSearchAline.Value), Sys.filterSQLStr(txtFlightNumber.Value), Sys.filterSQLStr(txtPlaneType.Value),"", Sys.filterSQLStr(txtStartDate.Value), Sys.filterSQLStr(txtEndDate.Value));
                                    sSql += string.Format(strInsertDetail, 3, getAirlineIdByName(txtSearchAline3.Value), Sys.filterSQLStr(txtFlightNumber3.Value), Sys.filterSQLStr(txtPlaneType3.Value),"", Sys.filterSQLStr(txtStartDate3.Value), Sys.filterSQLStr(txtEndDate3.Value));
                                }
                                else if (isTransfer == 2)//往返二次中转
                                {
                                    sSql += string.Format(strInsertDetail, 0, getAirlineIdByName(txtSearchAline.Value), Sys.filterSQLStr(txtFlightNumber.Value), Sys.filterSQLStr(txtPlaneType.Value),"", Sys.filterSQLStr(txtStartDate.Value), Sys.filterSQLStr(txtEndDate.Value));
                                    sSql += string.Format(strInsertDetail, 1, getAirlineIdByName(txtSearchAline2.Value), Sys.filterSQLStr(txtFlightNumber2.Value), Sys.filterSQLStr(txtPlaneType2.Value), Sys.filterSQLStr(txtSearch_relayport2.Value), Sys.filterSQLStr(txtStartDate2.Value), Sys.filterSQLStr(txtEndDate2.Value));
                                    sSql += string.Format(strInsertDetail, 2, getAirlineIdByName(txtSearchAline5.Value), Sys.filterSQLStr(txtFlightNumber5.Value), Sys.filterSQLStr(txtPlaneType5.Value), Sys.filterSQLStr(txtSearch_relayport3.Value), Sys.filterSQLStr(txtStartDate5.Value), Sys.filterSQLStr(txtEndDate5.Value));
                                    ...
                                    sSql += string.Format(strInsertDetail, 3, getAirlineIdByName(txtSearchAline3.Value), Sys.filterSQLStr(txtFlightNumber3.Value), Sys.filterSQLStr(txtPlaneType3.Value),"", Sys.filterSQLStr(txtStartDate3.Value), Sys.filterSQLStr(txtEndDate3.Value));
                                    sSql += string.Format(strInsertDetail, 4, getAirlineIdByName(txtSearchAline4.Value), Sys.filterSQLStr(txtFlightNumber4.Value), Sys.filterSQLStr(txtPlaneType4.Value), Sys.filterSQLStr(txtSearch_relayport4.Value), Sys.filterSQLStr(txtStartDate4.Value), Sys.filterSQLStr(txtEndDate4.Value));
                                    sSql += string.Format(strInsertDetail, 5, getAirlineIdByName(txtSearchAline6.Value), Sys.filterSQLStr(txtFlightNumber6.Value), Sys.filterSQLStr(txtPlaneType6.Value), Sys.filterSQLStr(txtSearch_relayport5.Value), Sys.filterSQLStr(txtStartDate6.Value), Sys.filterSQLStr(txtEndDate6.Value));
                                }
                                else//往返一次中转
                                {
                                    sSql += string.Format(strInsertDetail, 0, getAirlineIdByName(txtSearchAline.Value), Sys.filterSQLStr(txtFlightNumber.Value), Sys.filterSQLStr(txtPlaneType.Value),"", Sys.filterSQLStr(txtStartDate.Value), Sys.filterSQLStr(txtEndDate.Value));
                        ...
                        {
                            SqlHelper.ExecuteNonQuery(SqlHelper.ConnString_insert, CommandType.Text, sSql);
                        }
                    }
                    catch (Exception)
                    {
                        scripthelp.Alert("添加机票信息时发生异常!", this.Page);
                        return;
                    }
                    scripthelp.Alert("添加机票信息成功!", this.Page);
                    #endregion
                }
