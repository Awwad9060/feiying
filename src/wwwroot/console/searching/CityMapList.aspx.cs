...
using Fei580.Model;
    ...
    public partial class CityMapList : System.Web.UI.Page
    {
        ...
        protected void GetCityMaplist()
        {

            DataSet ds = new DataSet();

            string str = @"select top  100 percent * from FY_CityMap  order by lineinter desc, cityName asc ";
        ...
        protected void btndel_Click(object sender, EventArgs e)
        {
            ...
            str.Append("delete from FY_CityMap where id in (" + Request.Form["chk"] + ");");
            ...
            using (SqlTransaction trans = conn.BeginTransaction())
            {
                try
                {
                    ...
                    trans.Commit();
                    scripthelp.AlertAndRedirect("操作成功！", "CityMapList.aspx", this.Page);
                }
                catch
                {
                    trans.Rollback();
                    scripthelp.AlertAndRedirect("操作失败！", "CityMapList.aspx:", this.Page);
                }
                finally
                {
                    conn.Close();
