...
public partial class console_person_feedbacklist : System.Web.UI.Page
{
    ...
    protected void GetShopNewslist()
    {
        ...
        string str = @"select top 100 percent * from FY_feedback left join tmember on  tmember.Mem_ID=FY_feedback.memberId where 1=1 ";

        if (txtsearch.Value.Trim() != "")
        {
            str += " and title like '%" + txtsearch.Value.Trim() + "%' or Mem_LoginName like '%" + txtsearch.Value.Trim() + "%'";
        }

        str += " order by fId desc";
    ...
    protected void btndel_Click(object sender, EventArgs e)
    {
        ...
        str.Append("delete from FY_feedback where fId in (" + Request.Form["chk"] + ");");
        ...
        using (SqlTransaction trans = conn.BeginTransaction())
        {
            try
            {
                ...
                trans.Commit();
                scripthelp.AlertAndRedirect("操作成功！", "feedbacklist.aspx", this.Page);
            }
            catch
            {
                trans.Rollback();
                scripthelp.AlertAndRedirect("操作失败！", "feedbacklist.aspx", this.Page);
            }
            finally
            {
                conn.Close();
