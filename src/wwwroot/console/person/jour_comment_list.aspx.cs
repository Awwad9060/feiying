...
public partial class console_person_jour_comment_list : System.Web.UI.Page
{
    ...
    protected void GetShopNewslist()
    {
        ...
        string str = @"select top 100 percent FY_member_article_comment.*,FY_member_article.title from FY_member_article_comment left join FY_member_article on FY_member_article_comment.artId=FY_member_article.aId where 1=1 ";

        if (txtsearch.Value.Trim() != "")
        {
            str += " and FY_member_article.title like '%" + txtsearch.Value.Trim() + "%'";
        }

        str += " order by FY_member_article_comment.addtime desc";
    ...
    protected void btndel_Click(object sender, EventArgs e)
    {
        ...
        str.Append("delete from FY_member_article_comment where id in (" + Request.Form["chk"] + ")");
        ...
        using (SqlTransaction trans = conn.BeginTransaction())
        {
            try
            {
                ...
                trans.Commit();
                scripthelp.AlertAndRedirect("操作成功！", "jour_comment_list.aspx", this.Page);
            }
            catch
            {
                trans.Rollback();
                scripthelp.AlertAndRedirect("操作失败！", "jour_comment_list.aspx", this.Page);
            }
            finally
            {
                conn.Close();
