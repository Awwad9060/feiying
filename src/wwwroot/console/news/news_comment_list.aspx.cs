...
public partial class console_news_news_comment_list : System.Web.UI.Page
{
    ...
    protected void GetShopNewslist()
    {
        ...
        string str = @"select top 100 percent FY_news_comment.*,FY_news.title from FY_news_comment left join FY_news on FY_news_comment.newId=FY_news.id where 1=1 ";

        if (txtsearch.Value.Trim() != "")
        {
            str += " and FY_news.title like '%" + txtsearch.Value.Trim() + "%'";
        }

        str += " order by FY_news_comment.addtime desc";
    ...
    protected void btndel_Click(object sender, EventArgs e)
    {
        ...
        str.Append("delete from FY_news_comment where id in (" + Request.Form["chk"] + ")");
        ...
        using (SqlTransaction trans = conn.BeginTransaction())
        {
            try
            {
                ...
                trans.Commit();
                scripthelp.AlertAndRedirect("操作成功！", "news_comment_list.aspx", this.Page);
            }
            catch
            {
                trans.Rollback();
                scripthelp.AlertAndRedirect("操作失败！", "news_comment_list.aspx", this.Page);
            }
            finally
            {
                conn.Close();
