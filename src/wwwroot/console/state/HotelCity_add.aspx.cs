...
public partial class console_state_HotelCity_add : System.Web.UI.Page
{
    static int id = 0;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            if (Request.QueryString["id"] != null)
            {
                ...
                string commandText1 = "select * from FY_HotelCity where tId='" + id + "'";
                ...
                this.lblTitle.Text = "修改酒店城市信息";
            }
            else
            {
                id = 0;
                this.lblTitle.Text = "添加酒店城市信息";
            }
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        ...
        if (id == 0)
        {
            ...
            string commandText = "INSERT INTO FY_HotelCity(cityCode,enName,chName,sId) values('" + cityCode + "','" + enName + "','" + chName + "','" + bigId + "')";
            int count = SqlHelper.ExecuteNonQuery(SqlHelper.ConnString_insert, CommandType.Text, commandText);
            
            if (count > 0)
            {
                scripthelp.AlertAndRedirect("添加酒店城市信息成功!", "HotelCity_list.aspx", this.Page);
            }
            else
            {
                scripthelp.Alert("添加酒店城市失败!", this.Page);
                return;
            }
        }
        else
        {
            ...
            string bigId = DropDownList1.SelectedValue;
            string commandText = "update FY_HotelCity set cityCode='" + cityCode + "' ,enName='" + enName + "' ,chName='" + chName + "' ,sId='" + bigId + "' where tId='" + id + "'";
            int count = SqlHelper.ExecuteNonQuery(SqlHelper.ConnString_insert, CommandType.Text, commandText);
            if (count > 0)
            {
                scripthelp.AlertAndRedirect("修改酒店城市信息成功!", "HotelCity_list.aspx", this.Page);
            }
            else
            {
                scripthelp.Alert("修改酒店城市信息失败!", this.Page);
                return;
            }
