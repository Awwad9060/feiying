...
public partial class console_airplan_airplan_comment_list : System.Web.UI.Page
{
    ...
    protected void GetShopNewslist()
    {
        ...
        string str = @"select top 100 percent FY_Airplan_comment.*,FY_Airplan.chName from FY_Airplan_comment left join FY_Airplan on FY_Airplan_comment.airplanId=FY_Airplan.pId where 1=1 ";

        if (txtsearch.Value.Trim() != "")
        {
            str += " and FY_Airplan.chName like '%" + txtsearch.Value.Trim() + "%'";
        }

        str += " order by FY_Airplan_comment.addtime desc";
    ...
    protected void btndel_Click(object sender, EventArgs e)
    {
        ...
        str.Append("delete from FY_Airplan_comment where id in (" + Request.Form["chk"] + ")");
        ...
        using (SqlTransaction trans = conn.BeginTransaction())
        {
            try
            {
                ...
                trans.Commit();
                scripthelp.AlertAndRedirect("操作成功！", "airplan_comment_list.aspx", this.Page);
            }
            catch
            {
                trans.Rollback();
                scripthelp.AlertAndRedirect("操作失败！", "airplan_comment_list.aspx", this.Page);
            }
            finally
            {
                conn.Close();
