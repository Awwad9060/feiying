...
public partial class console_airplan_airplan_list : System.Web.UI.Page
{
    ...
    protected void GetShopNewslist()
    {
        ...
        string str = @"select top 100 percent * from FY_Airplan where 1=1 ";

        if (txtsearch.Value.Trim() != "")
        {
            str += " and chName like '%" + txtsearch.Value.Trim() + "%'";
        }
    ...
    protected void btndel_Click(object sender, EventArgs e)
    {
        ...
        str.Append("delete from FY_Airplan where pId in (" + Request.Form["chk"] + ");");
        ...
        using (SqlTransaction trans = conn.BeginTransaction())
        {
            try
            {
                ...
                trans.Commit();
                scripthelp.AlertAndRedirect("操作成功！", "airplan_list.aspx", this.Page);
            }
            catch
            {
                trans.Rollback();
                scripthelp.AlertAndRedirect("操作失败！", "airplan_list.aspx", this.Page);
            }
            finally
            {
                conn.Close();
