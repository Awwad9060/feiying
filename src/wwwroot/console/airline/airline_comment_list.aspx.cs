...
public partial class console_airline_airline_comment_list : System.Web.UI.Page
{
    ...
    protected void GetShopNewslist()
    {
        ...
        string str = @"select top 100 percent FY_AirLine_comment.*,FY_Airline.chName from FY_AirLine_comment left join FY_Airline on FY_AirLine_comment.airlineId=FY_Airline.aId where 1=1 ";

        if (txtsearch.Value.Trim() != "")
        {
            str += " and FY_Airline.chName like '%" + txtsearch.Value.Trim() + "%'";
        }

        str += " order by FY_AirLine_comment.addtime desc";
    ...
    protected void btndel_Click(object sender, EventArgs e)
    {
        ...
        str.Append("delete from FY_AirLine_comment where id in (" + Request.Form["chk"] + ")");
        ...
        using (SqlTransaction trans = conn.BeginTransaction())
        {
            try
            {
                ...
                trans.Commit();
                scripthelp.AlertAndRedirect("操作成功！", "airline_comment_list.aspx", this.Page);
            }
            catch
            {
                trans.Rollback();
                scripthelp.AlertAndRedirect("操作失败！", "airline_comment_list.aspx", this.Page);
            }
            finally
            {
                conn.Close();
