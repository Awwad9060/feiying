...
public partial class console_guest_leaveTel : System.Web.UI.Page
{
    ...
    {
        string str = "delete from FY_GuestTel where gId=" + Convert.ToInt32(Request.QueryString["Id"]);
        int deltag = 0;
        deltag = SqlHelper.ExecuteNonQuery(SqlHelper.ConnString_insert, CommandType.Text, str);
        if (deltag > 0)
            scripthelp.AlertAndRedirect("删除成功", "leaveTel.aspx", Page);
        else
            scripthelp.AlertAndRedirect("删除失败", "leaveTel.aspx", Page);
