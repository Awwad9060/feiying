...
public partial class console_guest_fastBook : System.Web.UI.Page
{
    ...
    protected void InitEntType()
    {
        ...
        string str = @"select top 100 percent * from FY_FastBook where 1=1";

        str += " order by bId desc";
    ...
    {
        string str = "delete from FY_FastBook where bId=" + Convert.ToInt32(Request.QueryString["Id"]);
        int deltag = 0;
        deltag = SqlHelper.ExecuteNonQuery(SqlHelper.ConnString_insert, CommandType.Text, str);
        if (deltag > 0)
            scripthelp.AlertAndRedirect("删除成功", "fastBook.aspx", Page);
        else
            scripthelp.AlertAndRedirect("删除失败", "fastBook.aspx", Page);
